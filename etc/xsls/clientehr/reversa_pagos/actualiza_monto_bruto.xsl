...
<xsl:stylesheet version="1.0"
	...
	<xsl:variable name="total_amount" select="/request-context/request/message/body/total_amount"/>
	...
	<xsl:template match="/">
		...
		<total_amount>
			<!-- <xsl:variable name="monto_bruto_sr" select="$total_amount"/>
			<xsl:variable name="monto_bruto" select="round($monto_bruto_sr * 100) div 100"/>
			 -->
			<bruto><xsl:value-of select="$total_amount"/></bruto>
