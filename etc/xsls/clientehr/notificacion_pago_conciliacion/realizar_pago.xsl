...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="/">
		...
		<transmission>
			...
			<!--Necesito el doc_id, asumo que viene, si no viene lo dejo en blanco-->
			 ...
			 <data>				
				 INSERT INTO PAGO(
					...
					tipo_servicio)
					values (
					'<xsl:value-of select="/request-context/services-responses/notificacion_pago_conciliacion/consulta_pag_id/response/data/Results/Row/NEXTVAL" />'
					,'<xsl:value-of select="$request_message/header/id_trx_rec" />'
					,'<xsl:value-of select="/request-context/services-responses/notificacion_pago_conciliacion/id_trx_pago_eft/response/data/Results/Row/NEXTVAL" />'
					,SYSDATE
