							...
							<xsl:when test="/request-context/request/message/body/payment_type = 'PM'">
								...
								<data>
									...
									<xsl:for-each select="$payment_request">
										...
										<!--comisiones e impuestos para pagos mixtos-->																			
										<!--Montos para los pagos mixtos-->
										        ...
										        INTO PAGO(
										              ...
										              values (
										                ...
										                ,'<xsl:value-of select="$payment[$position]/Results/Row/ID_CLIENTE" />'
 												        ,'<xsl:value-of select="$actualiza_monto_bruto[$position]/medio_pago[$position_data]/monto_neto"/>'<!--Se-->
										                ,'<xsl:value-of select="$actualiza_monto_bruto[$position]/medio_pago[$position_data]/monto_bruto"/>'<!--Se-->
										                ...
										                ,1 <!--El pago se inserta como no notificado-->
									...
									INSERT ALL
										...
										<xsl:for-each select="$payment">
									        ...
									        INTO PAGO(
									              ...
									              ,id_cliente
										      ,monto_neto
									              ,monto_pago<!--Se esta guardando el monto en bruto-->
									              ,moneda
									              ,numero_decimales
									              ,canal
									              ,medio_pago
 											...
 											,'<xsl:value-of select="$montos_actualizados[position()]/monto_neto"/>'<!--Se-->
									                ,'<xsl:value-of select="$montos_actualizados[position()]/monto_bruto"/>'<!--Se va a guardar el monto en bruto, pero tambien tenemos disponible el neto-->
									                ,'<xsl:value-of select="$request_message/header/currency_code" />'
									                ,'<xsl:value-of select="$request_message/body/decimal_places" />'
									                ,'<xsl:value-of select="$request_message/body/channel" />'
