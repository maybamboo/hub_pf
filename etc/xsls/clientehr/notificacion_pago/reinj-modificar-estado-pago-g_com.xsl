...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="/">
		...
		<reinject>
			...
			<id>
				<xsl:value-of
					select="concat('r-r-p',/request-context/services-responses/notificacion_pago/id_trx_eft/response/data/Results/Row/NEXTVAL)" />
			</id>
						
				<wait>
					<period from="0" to="300" each="20" />
				</wait>
				<max-intends>10</max-intends>
				<cut-hour></cut-hour>
			...
			<message>
				...
				<message>
					...
					<header>
						<msg_type>reinj-add-pago</msg_type>
					</header>
						...
						<xsl:choose>
							<xsl:when test="/request-context/request/message/body/payment_type = 'PM'">
								...
								<data>
									...
									<xsl:variable name="actualiza_monto_bruto" select="/request-context/services-responses/notificacion_pago/actualiza_monto_bruto/pagos/pago"/>
											...
											<xsl:if test="substring-before(type,'-') = 'MP'">
										        ...
										        INTO PAGO(
										              ...
										              values (
										                ...
										                ,'<xsl:value-of select="$actualiza_monto_bruto[$position]/medio_pago[$position_data]/monto_bruto"/>'<!--Sale del dato adicional,AQUI-->
										                ...
										                ,1 <!--Se inserta el pago como rechazado-->
										                ,1
										                ...
										                ,'<xsl:value-of select="$montos_actualizados[$position]/medio_pago[$position_data]/comision"/>'
										                ,'<xsl:value-of select="$montos_actualizados[$position]/medio_pago[$position_data]/impuesto"/>'
										...
										<xsl:for-each select="$payment">
									        ...
									        INTO PAGO(
									              ...
									              values (
									                ...
									                ,'<xsl:value-of select="$montos_actualizados[position()]/monto_bruto"/>'<!--Se usa el monto del pago que viene desde la peticion-->
									                ,'<xsl:value-of select="$request_message/header/currency_code" />'
									                ,'<xsl:value-of select="$request_message/body/decimal_places" />'
									                ,'<xsl:value-of select="$request_message/body/channel" />'
									                ,'<xsl:value-of select="$request_message/body/payment_method" />'
									                ,1 <!--El pago se inserta como rechazado-->
									                ,1
									                ...
									                ,4
									                ,1
									                ,'<xsl:value-of select="Results/Row/FECHA_VCTO" />'
									                ,'<xsl:value-of select="$montos_actualizados[position()]/comision"/>'
									                ,'<xsl:value-of select="$montos_actualizados[position()]/impuesto"/>'
									                ,'<xsl:value-of select="/request-context/services-responses/default/convenio/msg/empresa_holding/cod_emp" />')
							  			</xsl:for-each>
