	...
	<xsl:template match="/">	
	   <message>
			...
			<body>
				...
				<xsl:variable name="pagos_parciales" select="/request-context/services-responses/notificacion_pago/valida_pagos_parciales/response" />
					...
					<payment>
						...
						<xsl:choose>
							...
							<xsl:when test="$estado = '1'">
								<status_code>CEF113</status_code>
								<status_desc>El pago corresponde a un pago parcial, no permitido en el convenio</status_desc>
							</xsl:when>
							<xsl:otherwise>
