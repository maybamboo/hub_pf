	...
	<xsl:template match="/">	
	   <message>
			...
			<body>
				...
				<xsl:variable name="pagos_vencidos" select="/request-context/services-responses/notificacion_pago/valida_pagos_vencidos/response" />
				...
				<xsl:for-each select="$payment">
					<payment>
						...
						<xsl:variable name="estado" select="substring($pagos_vencidos,position(),1)"/>
						<xsl:choose>
							<!--La deuda no existe-->
							<xsl:when test="$estado = '1'">
								<status_code>CEF115</status_code>
								<status_desc>La deuda esta vencida, no se permite su pago en el convenio</status_desc>
							</xsl:when>
							<xsl:otherwise>
