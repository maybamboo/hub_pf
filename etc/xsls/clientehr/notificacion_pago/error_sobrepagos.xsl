	...
	<xsl:template match="/">	
	   <message>
			...
			<body>
				...
				<xsl:variable name="sobrepagos" select="/request-context/services-responses/notificacion_pago/valida_sobrepagos/response" />
				...
				<xsl:for-each select="$payment">
					<payment>
						...
						<xsl:variable name="estado" select="substring($sobrepagos,position(),1)"/>
						<xsl:choose>
							<!--La deuda no existe-->
							<xsl:when test="$estado = '1'">
								<status_code>CEH13</status_code>
								<status_desc>El pago corresponde a un sobrepago, no permitido en el convenio</status_desc>
							</xsl:when>
							<xsl:otherwise>
