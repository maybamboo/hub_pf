	...
	<xsl:template match="/">	
	   <message>
			...
			<body>
				...
				<xsl:variable name="pagos_mixtos" select="/request-context/services-responses/notificacion_pago/validar_pagos_mixtos/response" />
				...
				<xsl:for-each select="$payment">
					<payment>
						...
						<xsl:variable name="estado" select="substring($pagos_mixtos,position(),1)"/>
						<xsl:choose>
							<!--La deuda no existe-->
							<xsl:when test="$estado = '1'">
								<status_code>CEF112</status_code>
								<status_desc>La suma de los medios de pago no coincide con el total (PM)</status_desc>
							</xsl:when>
							<xsl:otherwise>
