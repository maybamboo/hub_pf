	...
	<xsl:template match="/">	
	   <message>
			...
			<body>
				...
				<status_desc>Pago no contemplado satisfactoriamente</status_desc>
				...
				<xsl:for-each select="$payment">
					<payment>
						...
						<!--Se devuelve el DOC_ID obtenido de la base de datos, independiente de que no se haya enviado en la SNP01-->
						<document_id><xsl:value-of select="Results/Row/DOC_ID"/></document_id>
						<!--Esto lo podemos hacer a través de un template-->
						<xsl:choose>
							<!--La deuda no existe-->
							<xsl:when test="ESTADO_PAGO = 1">
